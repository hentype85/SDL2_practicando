...
void Game::render() {
    ...
    // dibujar linea direccional del rectangulo
    SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255); // color blanco
    // longitud para la linea
    int LINE_LENGTH = 50;
    // calcula la diferencia en las coordenadas x e y entre el centro del rectangulo y la posicion del mouse
    int dx = sRect.mousePosX - (sRect.x + sRect.width / 2);
    int dy = sRect.mousePosY - (sRect.y + sRect.height / 2);
    // calcula la distancia entre el centro del rectangulo y la posicion del mouse
    float distance = sqrt(dx * dx + dy * dy);
    // calcula las coordenadas finales de la linea
    int endX = sRect.x + sRect.width / 2 + (dx / distance) * LINE_LENGTH;
    int endY = sRect.y + sRect.height / 2 + (dy / distance) * LINE_LENGTH;
    // dibuja la línea desde el centro del rectangulo hasta las coordenadas finales
    SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255); // color blanco
    SDL_RenderDrawLine(renderer, sRect.x + sRect.width / 2, sRect.y + sRect.height / 2, endX, endY);
